...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    updateLanguage();
    // Listen for storage changes if language is toggled elsewhere
  ...
  const fetchDashboardData = async () => {
    ...
    // 2. Fetch Entities for Receivables / Payables
    const { data: entities } = await supabase.from('entities').select('*');

    // 3. Fetch Products for Stock Alerts & Profit Calc
    const { data: products } = await supabase.from('products').select('*');

    // 4. Fetch Transaction Items for Profit Calculation
    ...
    // Profit Calculation (Sale Item Price - Purchase Price * Qty)
    let profit = 0;
            ...
            <table className="w-full text-sm text-left rtl:text-right">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {recentTransactions.map((tx) => (
                  <tr key={tx.id}>
                    <td className="p-3 font-mono font-bold text-blue-600">{tx.invoice_number}</td>
                    <td className="p-3">
                      {tx.type === 'sale' ? (
                    ...
                    <td className="p-3 font-medium">{tx.entities?.name || t.walkIn}</td>
